...
public:
  ...
  results_type resolve(implementation_type&, const query_type& queryIn,
      asio::error_code& ec)
  {
    asio::detail::addrinfo_type* address_info = 0;

    socket_ops::getaddrinfo(queryIn.host_name().c_str(),
        queryIn.service_name().c_str(), queryIn.hints(), &address_info, ec);
    auto_addrinfo auto_address_info(address_info);

    return ec ? results_type() : results_type::create(
        address_info, queryIn.host_name(), queryIn.service_name());
  }

  // Asynchronously resolve a query to a list of entries.
  template <typename Handler, typename IoExecutor>
  void async_resolve(implementation_type& impl, const query_type& queryIn,
      Handler& handler, const IoExecutor& io_ex)
  {
    ...
    p.p = new (p.v) op(impl, queryIn, scheduler_, handler, io_ex);
